...
const student = async (req, res) => {
  try {
    ...
    let getStudent = await studentModel
      ...
      .select({ updatedAt: 0, __v: 0, createdAt: 0 });
    return res.status(201).send({ status: true, ["student data"]: getStudent });
  } catch (error) {
...
const deleteStudent = async function (req, res) {
  try {
    ...
    if (!student) return res.send({status: false,message: "Student with this StudentId does not exist"});
    
   
    
    await studentModel.findOneAndUpdate(studentId, {isDeleted: true});
